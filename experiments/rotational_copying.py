...
from rotational_model import RotationalGPT, ModelConfig
from tokenizer import Tokenizer
from rotational_trainer import TrainConfig, train
...
class CopyingConfig:
    ...
    n_heads: int = 4
    embed_dim: int = 128
    train: TrainConfig = field(default_factory=lambda: TrainConfig(lr=0.1))
...
def evaluate(tok: Tokenizer, cfg: CopyingConfig, model: RotationalGPT, device: str, n_samples: int = 500) -> dict:
    correct = 0
...
def interactive(tok: Tokenizer, cfg: CopyingConfig, model: RotationalGPT, device: str):
...
def main():
    ...
    model = RotationalGPT(model_cfg)

    from rotational_trainer import get_device
